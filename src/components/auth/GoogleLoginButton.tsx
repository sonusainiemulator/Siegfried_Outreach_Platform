  ...
  const triggerGoogleLogin = () => {
    ...
    if (!tokenClientRef.current && window.google?.accounts?.oauth2) {
      try {
        ...
        console.error('Google token client fallback init error:', e)
      }
    }
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <button
        ...
        disabled={isLoading}
        className="w-full h-12 rounded-full border border-gray-200 dark:border-white/10 bg-white dark:bg-black/40 hover:bg-gray-50 dark:hover:bg-white/10 text-gray-700 dark:text-white flex items-center justify-center gap-3 text-[15px] font-medium transition-all shadow-xs cursor-pointer active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {isLoading ? (
          <>
            <div className="w-5 h-5 border-2 border-gray-400 border-t-primary rounded-full animate-spin" />
            <span>{t('signing_in_google', { defaultValue: 'Signing in with Google...' })}</span>
          </>
        ) : (
          <>
