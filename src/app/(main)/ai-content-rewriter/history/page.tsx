...
import { useGetRewriterHistoryQuery } from '@/redux/api/smartWriterApi'
import { downloadFile } from '@/utils/download'
import { useState } from 'react'
import { useTranslation } from 'react-i18next'
import { toast } from 'sonner'

const RewriterHistoryPage = () => {
  ...
  const { data: historyData, isLoading, isFetching } = useGetRewriterHistoryQuery({ page, limit: 10 })
  ...
  const handleDownload = (item: any) => {
    ...
    downloadFile(url, `${item.title || 'rewritten-content'}.txt`)
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar">
        <div className="max-w-[1600px] mx-auto">
          {filteredHistory.length > 0 || isLoading ? (
            ...
            <HistoryEmptyState startRoute="/content-rewriter-ai" />
          )}
        </div>
      </main>
...
export default RewriterHistoryPage
