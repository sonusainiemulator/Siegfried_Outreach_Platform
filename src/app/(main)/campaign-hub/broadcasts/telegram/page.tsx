...
const TelegramCampaignsPage = () => {
  const { t } = useTranslation()
  const router = useRouter()
  const { hasPermission } = usePermission()
  const canManage = hasPermission(PERMISSIONS.TELEGRAM_BROADCAST, 'write') || hasPermission(PERMISSIONS.MANAGE_BROADCASTS, 'write')
  ...
  const campaigns = data?.broadcasts || []

  const handleEdit = (campaign: Campaign) => router.push(`${ROUTES.CAMPAIGN_HUB.BROADCASTS.TELEGRAM}/${campaign.id}`)
  ...
  const columns: Column<Campaign>[] = [
    ...
    },
    {
      ...
      cell: (campaign) => {
        ...
        return (
          <span
            className={`px-2 py-1 rounded-full text-[10px] uppercase tracking-wider font-bold ${getStatusStyles(status)}`}
          >
            {t(status)}
          </span>
        )
    ...
    },
    {
      ...
      cell: (campaign) => {
        ...
        return (
          <span className="text-sm text-muted-foreground">{isValidDate ? format(date, 'dd-MM-yyyy hh:mm a') : t('instantly', { defaultValue: 'Instantly' })}</span>
        )
      },
  ...
  return (
    <>
      <TableLayout
        title={t('telegram_campaigns')}
        subtitle={t('manage_telegram_campaigns_desc')}
        primaryAction={
          canManage
            ? {
                label: t('add_campaign'),
                onClick: () => router.push(`${ROUTES.CAMPAIGN_HUB.BROADCASTS.TELEGRAM}/create`),
...
export default TelegramCampaignsPage
