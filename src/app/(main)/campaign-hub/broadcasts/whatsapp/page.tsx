...
const WhatsAppCampaignsPage = () => {
  ...
  const canManage = hasPermission(PERMISSIONS.WHATSAPP_BROADCAST, 'write') || hasPermission(PERMISSIONS.MANAGE_BROADCASTS, 'write')
  ...
  const handleEdit = (campaign: Campaign) => router.push(`${ROUTES.CAMPAIGN_HUB.BROADCASTS.WHATSAPP}/${campaign.id}`)
  ...
  const columns: Column<Campaign>[] = [
    ...
    },
    {
      ...
      cell: (campaign) => {
        ...
        return (
          <span
            className={`px-2 py-1 rounded-full text-[10px] uppercase tracking-wider font-bold ${getStatusStyles(
              status,
            )}`}
          >
            {t(status)}
  ...
  return (
    <>
      <TableLayout
        title={t('whatsapp_campaigns')}
        subtitle={t('manage_whatsapp_campaigns_desc')}
        primaryAction={
          canManage
            ? {
                label: t('add_campaign'),
                onClick: () => router.push(`${ROUTES.CAMPAIGN_HUB.BROADCASTS.WHATSAPP}/create`),
